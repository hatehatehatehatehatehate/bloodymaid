document.addEventListener('DOMContentLoaded', () => {
    ...
    // Клонирование элементов для создания бесконечного эффекта
    const firstItemClone = items[0].cloneNode(true);
    const lastItemClone = items[items.length - 1].cloneNode(true);
    track.appendChild(firstItemClone);
    track.insertBefore(lastItemClone, track.firstChild);
    let currentItemIndex = 1; // Начинаем с первого "реального" элемента, с учётом добавленного последнего элемента в начало

    track.style.transform = `translateX(${-slideWidth * currentItemIndex}px)`; // Инициализация позиции

    function switchItem() {
        if (currentItemIndex >= items.length + 1) { // Условие для сброса к началу
            track.style.transition = 'none'; // Отключаем анимацию для сброса
            currentItemIndex = 1; // Сброс к "реальному" первому элементу
            track.style.transform = `translateX(${-slideWidth * currentItemIndex}px)`;
            setTimeout(() => { // Возвращаем анимацию после мгновенного сброса
                track.style.transition = 'transform 0.5s ease';
            });
        } else {
            track.style.transition = 'transform 0.5s ease'; // Убеждаемся, что анимация включена
            currentItemIndex++;
            track.style.transform = `translateX(${-slideWidth * currentItemIndex}px)`;
        }
    }
